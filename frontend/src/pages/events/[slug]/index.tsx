...
const Event = () => {
		...
		const fetchData = async () => {
			try {
				const res = await axiosClient.get(
					"/hackathon/view/" + router.query.slug
				);
	...
	return (
		<>
				...
				>
					<Container className="d-flex justify-content-start align-items-center flex-row mt-2 border-none">
						<HighlightItem
							icon={faClock}
							name="Start Date"
							value={moment(eventData.event_date).format("DD/MM/YY HH:mm")}
						/>

						<HighlightItem
							icon={faClock}
							name="Registration Deadline"
							value={moment(eventData.registration_end).format(
								"DD/MM/YY HH:mm"
							)}
						/>
						...
						<HighlightItem
							icon={faMapMarker}
							name="Location"
							value={eventData.online ? "Online" : eventData.venue}
						/>

						<span className="ms-auto px-3 text-secondary">
							{eventData.registration_count} registered
						</span>

						{!teamData && (
							<Link href={`/events/${eventData.slug}/register`}>
								{" "}
								<Button className="me-3">{"Register Now"}</Button>
							</Link>
						)}
					...
					<div className="mb-3 mt-2">
						...
						{teamData && !teamData.shortlisted && (
							<Link
								href={
									teamData?.screening_submitted
										? "#"
										: `/events/${router.query.slug}/abstractsub`
								}
							>
								<Button
									disabled={teamData?.screening_submitted}
									className="me-3"
								>
									<FontAwesomeIcon icon={faWpforms} className="me-2" />
									{teamData?.screening_submitted
										? "Already Submitted"
										: "Make Screening Submission"}
								</Button>
							</Link>
						)}
						...
						{eventData.ps_form_released && teamData && (
							<Link
								href={
									teamData?.ps_preferences
										? "#"
										: `/events/${router.query.slug}/preference`
								}
							>
								<Button disabled={teamData?.ps_preferences}>
									<FontAwesomeIcon icon={faWpforms} className="me-2" />
									{teamData?.ps_preferences
										? "PS Preference Filled"
										: "Fill PS Preferences"}
								</Button>
							</Link>
						)}
			...
			<div>
				<Container>
					<Card className="mb-2 shadow border-none rounded-3 p-4">
						<h3>About {eventData.name}</h3>
